...
```{r getdata, message=FALSE, warning=FALSE, echo=TRUE}
library(caret)
...
In random forests, there is no need for cross-validation or a separate test set to get an unbiased estimate of the test set error. Our Out-Of-Sample (OOS) accuracy is **`r round(oosAccuracy*100,2)`**. Our OOS error is 1 - accuracy or **`r round(oosError*100,2)`**. As we can see our model is very robust and has an extremely high OOS accuracy.
...
```{r treePlot, anchor='treePlot', echo=FALSE}
treePlot <- prp(tree)
```

###Figure 2
```{r modelPlot, anchor='modelPlot', echo=FALSE}
modelPlot
```

###Figure 3
```{r predictionPlot, anchor='predictionPlot', echo=FALSE}
predictionPlot
```
